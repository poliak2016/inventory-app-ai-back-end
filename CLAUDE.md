...
- products: `updateProduct` — `qUpdateProduct` does a hard `SET category_id = $9` (no `COALESCE`), and the service builds it as `category_id: categoryId ?? null` ([products.service.js:117](src/services/products.service.js#L117)) — an update that omits `categoryId` silently wipes an existing category, since omitted (`undefined`) and explicit-clear (`null`) both collapse to the same `null` before reaching SQL. Plain `COALESCE($9, category_id)` isn't a full fix either: it would stop the silent wipe on omit, but then also block ever intentionally clearing a category (both cases arrive at SQL as `NULL`, indistinguishable). Real fix needs to distinguish "key absent from request body" vs "key present as null" at the JS layer (e.g. `'categoryId' in productData`) before the query is built — likely a dynamically-constructed `SET` clause rather than a fixed COALESCE template. Same root cause could affect other nullable product columns (`avg_weight_grams`, `avg_volume_ml`) — worth checking once this is designed. Discovered 2026-09-05 during a code-reading exercise on `products.service.js`, not fixed — deliberately deferred to keep that session's actual plan (`product.schema.js` unit/quantity validation) on track.
